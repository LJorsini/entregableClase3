...
for (i=1; i<=5; i++) {
    let votaPor = parseInt(prompt("Por que partiodo desea votar:\n 1-Partido A\n 2-Partido B\n 3-Voto Blanco"));
    
        if (votaPor == 1) {
            votoA = votoA+1;
            porcentajeVotosA = parseFloat(votoA*100/5);
        }else if (votaPor == 2) {
            votoB = votoB + 1;
            porcentajeVotosB = votoB*100/5;
        }else if (votaPor == 3) {
            votoBlanco = votoBlanco + 1;
            porcentajeBlancos = votoBlanco*100/5;
        }
        else {
